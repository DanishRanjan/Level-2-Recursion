...
public class kSubsetWithEqualSum {
	...
	public static void solution(int[] arr, int vidx,int n , int k,int[] subsetSum,int ssssf, ArrayList<ArrayList<Integer>> ans) {
		...
		for(int i=0;i<ans.size();i++){
		      if(ans.get(i).size() > 0){ //ya to main existing non empty set mein baith jaau
		          ans.get(i).add(arr[vidx]);
		          subsetSum[i] += arr[vidx];
		          solution(arr, vidx + 1, n, k, subsetSum, ssssf, ans);
		          subsetSum[i] -= arr[vidx]; //backtracking
		          ans.get(i).remove(ans.get(i).size() - 1); //backtracking
		      }else{                  //ya main phela naya set start karu
		          ans.get(i).add(arr[vidx]);
		          subsetSum[i] += arr[vidx];
		          solution(arr, vidx + 1, n, k,subsetSum, ssssf + 1, ans);
		          subsetSum[i] -= arr[vidx]; //backtracking
		          ans.get(i).remove(ans.get(i).size() - 1);//backtracking 
		          break; // to stop repeatation
		      }  
		}
